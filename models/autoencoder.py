...
class GaussianDistribution:
    ...
    def kl(self):
        """
        https://github.com/CompVis/stable-diffusion/blob/main/ldm/modules/distributions/distributions.py#L24
        """
...
def gaussian_sample(parameters):
    # Split mean and log of variance
    mean, log_var = torch.chunk(parameters, 2, dim=1)
    # Clamp the log of variances
...
if __name__ == "__main__":
    ...
    autoencoder = Autoencoder(
        encoder=encoder, decoder=decoder, emb_channels=4, z_channels=4
    ).to(device=device)

    x = torch.rand(size=(1, 1, 256, 256)).to(device=device)
    enc = autoencoder.encode(x).sample()
    dec = autoencoder.decode(enc)
    torchinfo.summary(encoder, input_size=(2, 1, 256, 256))
    torchinfo.summary(decoder, input_size=(2, 4, 32, 32))
