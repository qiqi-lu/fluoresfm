...
df_metrics = {name: None for name in metrics_names}
...
# some image may need background subtraction to remove the effect of the
# high intensity background signal.
...
for id_dataset in params["dataset_names"]:
    print("-" * 80)
    print("[INFO] Dataset:", id_dataset)

    # CHECK WHETHER TO USE BACKGROUND SUBTRACTION
    if id_dataset in datasets_need_bkg_sub:
        sub_bkg = True
        print("[WARNNING] Background subtraction is applied.")
    else:
        sub_bkg = False
    ...
    if params["num_sample"] is not None:
        ...
        num_sample_analysis = len(sample_filenames)
    print(
        f"[INFO] Number of test samples: {num_sample_analysis}|{len(sample_filenames)}"
    )

    # --------------------------------------------------------------------------
    # load results in Excel file
    # --------------------------------------------------------------------------
    for name in metrics_names:
        try:
            df_metrics[name] = pandas.read_excel(
                path_metrics_file, sheet_name=name, index_col=0
            )
        except:
            df_metrics[name] = pandas.DataFrame()
    ...
    try:
        ...
        for i_sample in range(num_sample_analysis):
            ...
            imgs_all.append(img_raw)

            # load estimated images form different methods ---------------------
            for meth in methods:
                ...
                imgs_all.append(img_tmp)

            imgs_all.append(img_gt)  # add ground truth to the list of images.
            # [raw, est1, est2, ..., gt] images
            num_image_all = len(imgs_all)

            # ------------------------------------------------------------------
            # evaluate all the images
            # ------------------------------------------------------------------
            metrics_method = []
            for i_img, img in enumerate(imgs_all):
                dict_tmp = {"img_true": img_gt, "img_test": img}
                if i_img != num_image_all - 1:  # not the ground truth image
                    ...
                    # soemtimes the size of image may be too small for MSSSIM calculation
                    try:
                        ...
                        try:
                            ...
                            traceback.print_exc()
                            msssim = 0
                    rse, rsp, _ = eva.SQUIRREL(img=img, img_ref=img_gt)
                else:
        ...
        assert metrics_dataset.shape[-1] == len(
            metrics_names
        ), "[ERROR] The number of calculated metrics is not equal to the expected number of metrics."

        # ----------------------------------------------------------------------
        all_titles = ["raw"] + titles + ["gt"]  # all titles of images
        for i_name, name in enumerate(metrics_names):
            for i_title, title in enumerate(all_titles):
                # if current method is not in the dataframe, add it at the last column
                if title not in list(df_metrics[name].columns):
                    df_metrics[name].insert(df_metrics[name].shape[-1], title, value="")
                # insert the metrics of current method into the dataframe
                df_metrics[name][title] = metrics_dataset[:, i_title, i_name]

        # ----------------------------------------------------------------------
        # save all the metrics value into a excel file
        for name in metrics_names:
            df_metrics[name].to_excel(writer, sheet_name=name, index_label="id")
        writer.close()
    except Exception as e:
        for name in metrics_names:
            df_metrics[name].to_excel(writer, sheet_name=name, index_label="id")
...
if datasets_not_processed:
    print("[INFO] The following datasets are not processed:")
    print(f"[INFO] {datasets_not_processed}")
else:
    print("[INFO] All datasets are processed successfully.")
print("-" * 80)
