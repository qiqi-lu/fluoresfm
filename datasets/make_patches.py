...
# patch_size, step_size = 128, 32
# patch_size, step_size = 128, 64
# patch_size, step_size = 192, 192
# patch_size, step_size = 256, 64
# patch_size, step_size = 256, 128
# patch_size, step_size = 384, 192

convert_3d_to_2d = True  # whether convert 3d to 2d pathces
enbale_zero_padding = False  # slightly zero padding

print("patch size:", patch_size, ", step size:", step_size)
print("Convert 3D images into 2D pathces:", convert_3d_to_2d)
print("Zero padding:", enbale_zero_padding)
...
for i in range(len(path_dataset)):
    ...
    for idx_sample in range(num_samples):
        ...
        if (dim == 2) or (dim == 3 and img.shape[0] == 1):
            ...
            # zero_padding
            if enbale_zero_padding:
                img = zero_padding(img, patch_size=patch_size, step_size=step_size)
        ...
        # 3D raw data
            ...
            if convert_3d_to_2d == False:
                if Nz <= patch_size:
                    if enbale_zero_padding:
                        img = zero_padding(
                            img, patch_size=patch_size, step_size=step_size
                        )
