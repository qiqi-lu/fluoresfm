"""
Display the results using finetuend model to do surface projection.
...
for i_sample in range(num_samples):
    ...
    img_gt = np.transpose(img_gt, (0, 2, 1))  # [z, x, y] -> [z, y, x]
    # img_gt = np.flip(img_gt, axis=1)  # up-down flip

    # load the raw image
    img_raw = io.imread(os.path.join(path_lr, filename)).astype(np.float32)
    img_raw = np.max(img_raw, axis=0, keepdims=True)  # maximum projection
    img_raw = interp_sf(img_raw, sf=sf_lr)
    img_raw = normalizer(img_raw)
    img_raw = np.clip(img_raw, **dict_clip)
    img_raw = np.transpose(img_raw, (0, 2, 1))  # [z, x, y] -> [z, y, x]
    # img_raw = np.flip(img_raw, axis=1)  # up-down flip
    ...
    for i_method in range(num_methods_show):
        ...
        img_pred = np.transpose(img_pred, (0, 2, 1))  # [z, x, y] -> [z, y, x]
        # img_pred = np.flip(img_pred, axis=1)  # up-down flip
