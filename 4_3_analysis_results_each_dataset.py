...
[columns.pop(-1) for _ in range(5)]
...
for metric in metrics:
    ...
    for task in tasks:
        ...
        # ----------------------------------------------------------------------
        pbar = tqdm.tqdm(total=num_dataset, desc=task.upper(), ncols=80)
        for dataset_name in dataset_names:
            # read excel of result from current dataset
            data_frame = pandas.read_excel(
                os.path.join(path_predictions, dataset_name, excel_file_name),
                sheet_name=metric,
            )
            ...
            mean_std_pvalue_frame.loc[idx, methods_targ + "-median"] = (
                data_targ.median()
            )
            ...
            for meth in methods_comp:
                ...
                mean_std_pvalue_frame.loc[idx, meth + "-median-diff"] = med_diff
                mean_std_pvalue_frame.loc[idx, meth + "-IC (low)"] = med_ic[0]
                mean_std_pvalue_frame.loc[idx, meth + "-IC (high)"] = med_ic[1]
            pbar.update(1)
        pbar.close()
